...
var arraySum = function(array) {
	let cv = array[array.length-1];
	if (Array.isArray(cv)){
	  return arraySum(cv) + arraySum(array.slice(0, array.length-1));
	}
	if (array.length <= 1){
		return array[0] || 0;
	}
	...
	if (!Array.isArray(cv)) {
      return cv + arraySum(array.slice(0, array.length-1))

	}
	// edge case: empty array, non-array
	//base case: last value in array, return value or 0 (edge case)
	//recursive case: if value is array, sum all parts, else return num + recursion
};
...
var exponent = function(base, exp) {
  ...
  if (exp < 0) return parseFloat(1/base * exponent(base, exp+1).toPrecision(3));
...
var reverse = function(string) {
	...
	if (string.length) return string[string.length-1] + reverse(string.slice(0, string.length-1))
};
...
var palindrome = function(string) {
  ...
  if (string.length === 2){
  	return string[0].toUpperCase() === string[1].toUpperCase()? true: false;
  }
  if (string.length > 2) {
...
var multiply = function(x, y) {
	if (x === 0|| y === 0){
      ...
      if (y - 1 > 0) return x + multiply(x, y-1);
      if (y === 1) return x;
	}
	else if (x < 0 && y < 0){
      if (y + 1 < 0) return -x + multiply(x, y+1);
...
var divide = function(x, y) {
	...
	if (x === 0|| y === 0){
      ...
      if (x - y >= 0) return 1 + divide(x - y, y);
      if (x - y < 0) return 0;
	}
	else if (x < 0 && y < 0){
      if (x - y <= 0) return 1 + divide(x - y, y);
...
var countOccurrence = function(array, value) {
  if (!array.length || arguments.length < 2) return null
  if (array.length === 1) return array[0] === value? 1 : 0;
  if (array.length) return array[0] === value?  1 + countOccurrence(array.slice(1), value): 0 + countOccurrence(array.slice(1), value);
};
...
var countKeysInObj = function(obj, key) {
	...
	for (let element in obj){
		...
		if (typeof obj[element] === 'object') count += countKeysInObj(obj[element], key);
	}
	return count;
};
...
var alternateSign = function(array) {
 ...
 if (array.length > 1){
 	result.push(array[array.length-1] * -1);
 	return alternateSign(array.slice(1)).concat(result);
 }
};
